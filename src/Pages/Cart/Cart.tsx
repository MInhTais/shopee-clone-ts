...
export default function Cart() {
    ...
    setExtendsPurchase((prev) => {
      ...
      return (
        productInCart?.map((purchase) => {
          console.log(extendedPurchaseObj[purchase._id])
  ...
  const handleMutation = (purchase_id: number) => () => {
    ...
    deletePurchaseMutation.mutate([product_id])
  }
  const handleDeleteManyPurchase = () => {
    const purchaseId = PurchaseChecked.map((purchase) => purchase._id)
    deletePurchaseMutation.mutate(purchaseId)
  }
  const buyPurchases = () => {
    const purchases = PurchaseChecked.map((purchase) => {
      return {
